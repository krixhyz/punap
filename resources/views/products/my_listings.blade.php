<x-app-layout>
    ...
    <div class="py-10 max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-12">
            ...
            <table class="w-full text-sm text-left text-gray-600 border-collapse">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3">Quantity</th> <!-- NEW -->
                ...
                <tbody>
                    @forelse ($products->where('status', '!=', 'sold') as $product)
                        <tr class="border-b hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">{{ $product->quantity }}</td> <!-- NEW -->
                            <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @if($rental->status === 'active')
                                <form action="{{ route('rental.return', $rental->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button class="bg-green-600 hover:bg-green-700 text-white text-xs px-3 py-1 rounded ml-1">
                                        Mark Returned
                                    </button>
                                </form>
                            @endif
                ...
                @foreach ($swapRequests as $swap)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 flex gap-2">
                            <a href="{{ route('swap.accept', $swap->id) }}" class="bg-green-600 hover:bg-green-700 text-white text-xs px-3 py-1 rounded">Accept</a>
                            <a href="{{ route('swap.reject', $swap->id) }}" class="bg-red-600 hover:bg-red-700 text-white text-xs px-3 py-1 rounded">Reject</a>
                        </td>
                    </tr>
                @endforeach
            </x-section-table>
        ...
        {{-- ==================== SECTION 6: Sold / Partially Sold Products ==================== --}}
        <div class="bg-white shadow-md rounded-2xl p-6 border border-gray-100">
            ...
            <table class="w-full text-sm text-left text-gray-600 border-collapse">
                <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-3">Remaining Qty</th> <!-- NEW -->
                        <th class="px-6 py-3">Total Revenue</th>
                        <th class="px-6 py-3">Status</th> <!-- NEW -->
                        <th class="px-6 py-3">Last Sale</th> <!-- NEW -->
                    </tr>
                </thead>
                <tbody>
                    @forelse ($soldProducts as $sold)
                        ...
                        <tr class="border-b hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">{{ $sold->quantity }}</td> <!-- remaining -->
